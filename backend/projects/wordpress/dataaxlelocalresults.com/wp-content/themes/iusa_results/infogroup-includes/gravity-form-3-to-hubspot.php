...
function gravity_form_3_to_hubspot($entry, $form) {
	...
	$options = array(
		...
		CURLOPT_POSTFIELDS => 'first_name=' . urlencode($firstName) . '&last_name=' . urlencode($lastName) . '&company=' . urlencode($businessName) . '&phone=' . urlencode($businessPhone) . '&email=' . urlencode($emailAddress) . '&media_code' . urlencode($mediacode) . '&bas_division=' . urlencode($basDivision) . '&page_url=' . urlencode($pageUrl) . '&originating_system=iusa' . '&hs_context=' . urlencode($hs_context_json),
        CURLOPT_POST => true,
