...
function iusa_register_or_login_user($validation_result) {
	...
	if ($regLastName == 'See First Name Field') {
		
		$splitFullName = explode(' ', $regFirstName, 2);

		if (isset($splitFullName[1], $splitFullName)) {

			$_POST['input_1'] = $splitFullName[0];

			$_POST['input_2'] = $splitFullName[1];

			$regFirstName = $splitFullName[0];

			$regLastName = $splitFullName[1];

		} else {			

			$_POST['input_2'] = 'No Last Name Given';

			$regLastName = 'No Last Name Given';
...
function iusa_registration_confirmation($confirmation, $form, $entry, $ajax) {
	...
	if (!empty($appReturnURL)) {
		...
		$iusaRegRedirectURL = 'https://dev-cms.infousa.com/resources/register/thank-you/';
		
	} elseif (CMS_ENVIRONMENT == 'TEST') {
		
		$iusaRegRedirectURL = 'https://test-cms.infousa.com/resources/register/thank-you/';		
		
	} elseif (CMS_ENVIRONMENT == 'PROD') {
		
		$iusaRegRedirectURL = 'https://www.infousa.com/resources/register/thank-you/';
