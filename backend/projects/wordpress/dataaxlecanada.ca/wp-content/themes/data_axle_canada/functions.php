...
function dataaxle_wp_get_attachment_image_src($image, $attachment_id, $size, $icon) {

    if (!empty($image)) {
        $changed_images = array();
        foreach ($image as $key => $value) {
            $changed_images[] = str_replace(site_url('/'), 'https://cdn.dataaxlecanada.ca/dev/', $value);
        }
...
/*add_filter('wp_get_attachment_image_src', 'dataaxle_wp_get_attachment_image_src', 10, 4);*/

function dataaxle_cdn_attachments_urls($url, $post_id) {

  return str_replace($_SERVER['REQUEST_SCHEME'].'://'.$_SERVER['HTTP_HOST'].'/wp-content/uploads', 'https://cdn.dataaxlecanada.ca/dev/wp-content/uploads', $url);
}
...
function to_cdn($src) {
   ...
   /* return $src_pre . 'cdn.' . $src_post;*/
    return str_replace($_SERVER['REQUEST_SCHEME'].'://'.$_SERVER['HTTP_HOST'].'/wp-content/uploads', 'https://cdn.dataaxlecanada.ca/dev/wp-content/uploads', $src);
}
