...
<html class="no-js" lang="en">
  <head>
    ...
    <?php 
      ...
      if (isset($_COOKIE['origin_lp'])) {

        $landingPageHomeURL = $_COOKIE['origin_lp'];

      } elseif (!empty($originLP)) {

        $landingPageHomeURL = $originLP;

      } else {

        $landingPageHomeURL = site_url();
    ...
    <?php
      ...
      if (get_post_type($post->ID) != 'sharedpages') {
        $originLP = get_the_permalink(); ?>  

      <script type="text/javascript">
        console.log('before lp cookie');
        Cookies.set('origin_lp', '<?php echo $originLP; ?>', { domain: '<?php echo COOKIEDOMAIN; ?>', path: '/lp/' });
        console.log('after lp cookie');
      </script>
    ...
    <script type="text/javascript">
      jQuery(document).ready(function() {
        jQuery('.home-url').click(function(event) {
          window.location = '<?php echo $landingPageHomeURL; ?>';
  ...
  <header id="site-header" role="banner" data-sticky-container>    
    ...
    <div id="logobar" class="row clearfix container1140">
      ...
      <div id="logo-left" class="small-12 medium-12 large-4 column">      
        ...
        <a id="main-logo-anchor" href="<?php echo $landingPageHomeURL; ?>"><img id="main-logo" src="<?php echo $infogroup_options['header-logo']['url']; ?>" alt="<?php echo get_bloginfo('name');?>&reg; from infogroup"></a>

        <div class="title-bar float-right" data-responsive-toggle="infogroup-main-nav" data-hide-for="large">

          <?php //if ($showNavMenu['0'] == 'Registration' || $showNavMenu['0'] == 'W2L') { ?>
      ...
      <div id="logo-right" class="small-12 medium-12 large-4 column">

        <div id="main-navbar">

          <?php
          
            main_landing_page_navigation()

            // if ($showNavMenu['0'] == 'Registration') {

            //   infogroup_main_navigation_reg();

            // } else if ($showNavMenu['0'] == 'W2L') {

            //   infogroup_main_navigation_w2l();
